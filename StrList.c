...
{
    if (StrList == NULL || StrList->_head == NULL) // Check if list or head is NULL
    {
        return;
    }
    if (index >= StrList->_size || index < 0) // Adjusted condition for index validity
    {
        return;
    }
    if (index == 0) // Special case for removing the head
    {
    ...
    {
        Node *current = StrList->_head;
        Node *currentPrev = NULL;       // Initialize to NULL
        for (int i = 0; i < index; i++) // Changed condition to index
        {
            currentPrev = current;
            current = current->_next;
        }
...
{
    if (StrList1 == NULL && StrList2 == NULL) // if they both  null
    {
        return 1;
    }
...
void StrList_reverse(StrList *StrList)
{
    ...
    Node *prev = NULL;
    // pointer to the StrList head's pointer
    Node *thisNode = StrList->_head;
    // Initiate a variable to store a pointer to the next node
    Node *next = NULL;
